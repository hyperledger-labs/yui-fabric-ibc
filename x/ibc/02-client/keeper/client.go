...
func (k Keeper) UpdateClient(ctx sdk.Context, clientID string, header exported.Header) (exported.ClientState, error) {
	...
	case fabrictypes.Fabric:
		clientState, consensusState, err = fabric.CheckValidityAndUpdateState(
			clientState, header, ctx.BlockTime(),
		)
	default:
	...
	if header != nil && clientType != exported.Localhost && (clientType == fabric.Fabric && !reflect.ValueOf(consensusState).IsNil()) {
